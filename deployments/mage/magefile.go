...
func CreateRelease() error {
	...
	githubClient := github.NewClient(oauth2.NewClient(ctx, oauth2.StaticTokenSource(&oauth2.Token{AccessToken: getGithubToken()})))

	_, resp, err := githubClient.Repositories.CreateRelease(ctx, getRepositoryOrg(), getRepositoryName(), getRepositoryRelease())
